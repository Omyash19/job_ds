...
st.markdown("""
    ...
    /* 1. Impact Branding (Larger & Fitted) */
    ...
    /* 2. Compact Filter Card (No Ghost Strips) */
    .filter-card {
    ...
    /* 3. Coordinated Primary Button (Small & Indigo) */
...
st.markdown('<div class="master-header"><h1 class="master-title">ECHOES 💎</h1><p class="master-subtitle">High Precision Data for the Modern Analyst</p></div>', unsafe_allow_html=True)
...
if not raw_df.empty:
    ...
    st.error("Engine failure. Please check database connectivity.")import streamlit as st
import pandas as pd
from sqlalchemy import create_engine
import plotly.express as px
import plotly.graph_objects as go
